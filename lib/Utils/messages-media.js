...
export const hkdfInfoKey = (type: string) => {
    const hkdfInfo = MEDIA_HKDF_KEY_MAPPING[type as keyof typeof MEDIA_HKDF_KEY_MAPPING];
    return `WhatsApp ${hkdfInfo} Keys`;
};

export const getRawMediaUploadData = async (media: any, mediaType: string, logger?: any) => {
...
export async function getMediaKeys(buffer: Buffer | string, mediaType: string) {
    if (!buffer) {
    ...
    const expandedMediaKey = await hkdf(buffer as Buffer, 112, { info: hkdfInfoKey(mediaType) });
    return {
...
const extractVideoThumb = async (path: string, destPath: string, time: string, size: { width: number }) => 
...
export const extractImageThumb = async (bufferOrFilePath: Buffer | string | Readable, width = 32) => {
    ...
    if ('sharp' in lib && typeof lib.sharp?.default === 'function') {
        const img = lib.sharp.default(bufferOrFilePath as Buffer | string);
        ...
        const jimp = await lib.jimp.Jimp.read(bufferOrFilePath as Buffer | string);
        const dimensions = {
            width: jimp.width,
            height: jimp.height
        };
...
export const generateProfilePicture = async (mediaUpload: any, dimensions?: { width?: number; height?: number }) => {
    let buffer: Buffer;
...
export const mediaMessageSHA256B64 = (message: any) => {
 ...
 */
export async function getAudioWaveform(buffer: Buffer | string | Readable, logger?: any) {
    try {
        // @ts-ignore
...
export const getStream = async (item: any, opts?: any) => {
...
export async function generateThumbnail(file: any, mediaType: string, options?: { logger?: any }) {
...
export const getHttpStream = async (url: string | URL, options: any = {}) => {
...
export const encryptedStream = async (
    media: any, 
    mediaType: string, 
    { logger, saveOriginalFileIfRequired, opts }: any = {}
) => {
    ...
    const onChunk = (buff: Buffer) => {
        ...
        if (!encFileWriteStream.write(buff)) {
            ...
            return new Promise<void>(resolve => {
                encFileWriteStream.once('drain', resolve);
            });
        }
...
export const downloadEncryptedContent = async (
    ...
    const pushBytes = (bytes: Buffer, push: (chunk: Buffer) => void) => {
    ...
    const output = new Transform({
        ...
        flush(callback) {
            try {
                ...
                // Clear remaining bytes to prevent memory leak
...
export function extensionForMediaMessage(message: any): string | undefined {
...
export const getWAUploadToServer = (
    { customUploadHosts, fetchAgent, logger, options }: any,
    refreshMediaConn: (forceRefresh?: boolean) => Promise<any>
) => {
    return async (filePath: string, { mediaType, fileEncSha256B64, timeoutMs }: any) => {
...
export const decryptMediaRetryData = async (
    { ciphertext, iv }: { ciphertext: Buffer; iv: Buffer },
    mediaKey: Buffer,
    msgId: string
) => {
    const retryKey = await getMediaRetryKey(mediaKey);
