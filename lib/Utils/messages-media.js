...
export const getHttpStream = async (url, options = {}) => {
    ...
    // Check if response.body exists and is a readable stream
    if (!response.body) {
        throw new Boom(`Response body is empty from ${url}`);
    }
...
// NEW FUNCTION: Stream-based file processing for large files
export const prepareStream = async (media, mediaType, { logger, saveOriginalFileIfRequired, opts } = {}) => {
    ...
    try {
        ...
        for await (const chunk of stream) {
            encFileWriteStream.write(chunk);
        }
        encFileWriteStream.end();
        await once(encFileWriteStream, 'finish');
        ...
        if (saveOriginalFileIfRequired) {
            ...
            // For file type, just use the original path
            if (type === 'file' && typeof media.url === 'string') {
                bodyPath = media.url;
            } else {
                // For buffer or remote, we need to save it
                const { stream: originalStream } = await getStream(media, opts);
                const originalWriteStream = createWriteStream(originalFilePath);
                
                try {
                    for await (const chunk of originalStream) {
                        originalWriteStream.write(chunk);
                    }
                    originalWriteStream.end();
                    await once(originalWriteStream, 'finish');
        ...
        // Get file size and hash without loading entire file
        const fileSize = (await fs.stat(encFilePath)).size;
        
        // Calculate SHA256 in streaming fashion
        const fileReadStream = createReadStream(encFilePath);
        ...
        const fileSha256 = sha256Hash.digest();
        stream.destroy();
...
// FIXED: Improved streaming for large files
        ...
        transform(chunk, _, callback) {
            try {
                ...
                if (data.length > 0) {
                    ...
                    // Handle byte range slicing efficiently
        ...
        flush(callback) {
            try {
                if (aes) {
                    ...
                    if (startByte || endByte) {
                        const start = bytesFetched >= startByte ? undefined : Math.max(startByte - bytesFetched, 0);
                        const end = bytesFetched + finalBytes.length < endByte ? undefined : Math.max(endByte - bytesFetched, 0);
                        const sliced = finalBytes.slice(start, end);
                        if (sliced.length > 0) {
                            this.push(sliced);
                        }
                    } else {
