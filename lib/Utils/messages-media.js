...
// FIXED: Add prepareStream function from second code
export const prepareStream = async (media, mediaType, { logger, saveOriginalFileIfRequired, opts } = {}) => {
    ...
    try {
        ...
        await new Promise((resolve, reject) => {
            stream.pipe(encFileWriteStream)
                .on('finish', resolve)
                .on('error', reject);
        });
        ...
        if (saveOriginalFileIfRequired) {
            ...
            const originalFileStream = createWriteStream(originalFilePath);
            
            // We need to re-read the stream or copy the file
            const originalStream = await getStream(media, opts);
            await new Promise((resolve, reject) => {
                originalStream.stream.pipe(originalFileStream)
                    .on('finish', resolve)
                    .on('error', reject);
            });
        ...
        // Calculate SHA256 using streams to avoid memory issues
        const hash = Crypto.createHash('sha256');
        const fileStream = createReadStream(encFilePath);
        for await (const chunk of fileStream) {
            hash.update(chunk);
        }
        const fileSha256 = hash.digest();
...
// FIXED: Modified upload function to handle streams better
    ...
    return async (filePath, { mediaType, fileEncSha256B64, timeoutMs }) => {
        ...
        for (const { hostname, maxContentLengthBytes } of hosts) {
            ...
            try {
                ...
                if (maxContentLengthBytes && fileSize > maxContentLengthBytes) {
