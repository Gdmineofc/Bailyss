...
// NEW: Stream-based upload data preparation
...
// NEW: Stream-based preparation without loading entire file into memory
export const prepareStream = async (media, mediaType, { logger, saveOriginalFileIfRequired, opts } = {}) => {
    ...
    try {
        // Stream the data to file without loading into memory
        const tmpFilePath = join(getTmpFilesDirectory(), mediaType + generateMessageIDV2() + '-tmp');
        const tmpWriteStream = createWriteStream(tmpFilePath);
        
        for await (const chunk of stream) {
            if (!tmpWriteStream.write(chunk)) {
                await once(tmpWriteStream, 'drain');
            }
        }
        
        tmpWriteStream.end();
        await once(tmpWriteStream, 'finish');
        
        // Get file size and hash using streams
        ...
        // If we need the original file
        if (saveOriginalFileIfRequired) {
            originalFilePath = tmpFilePath;
            bodyPath = tmpFilePath;
