...
export const getHttpStream = async (url, options = {}) => {
    // Simple streaming approach for Node.js
    return new Promise((resolve, reject) => {
        const http = url.toString().startsWith('https://') ? require('https') : require('http');
        
        const req = http.get(url.toString(), options, (res) => {
            if (res.statusCode !== 200) {
                reject(new Boom(`Failed to fetch stream from ${url}`, { 
