...
// Fix: Use dynamic imports for http/https modules in ES modules
export const getHttpStream = async (url, options = {}) => {
    ...
    if (globalThis.fetch) {
        ...
        if (!response.ok) {
            throw new Boom(`Failed to fetch stream from ${urlStr}`, { 
        ...
        // Fallback for Node.js < 18 - use dynamic imports
        const http = urlStr.startsWith('https://') 
            ? (await import('https')).default 
            : (await import('http')).default;
        
        return new Promise((resolve, reject) => {
            const req = http.get(urlStr, { ...options }, (res) => {
                if (res.statusCode !== 200) {
                    reject(new Boom(`Failed to fetch stream from ${urlStr}`, { 
...
export const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (filePath, { mediaType, fileEncSha256B64, timeoutMs }) => {
        ...
        for (const { hostname } of hosts) {
            ...
            let result;
            try {
                // Use fetch for uploads
                const stream = createReadStream(filePath);
                const formData = new FormData();
                
                // For Node.js 18+ with fetch
                if (globalThis.fetch) {
